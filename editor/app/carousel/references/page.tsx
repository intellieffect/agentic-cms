...
function slideImageUrl(postId: string, idx: number) {
  return `/api/ref-posts/${postId}/slides/${idx}/image`
}
...
export default function CarouselReferencesPage() {
  ...
  const fetchAccounts = useCallback(async () => {
    try {
      const data = await cachedFetch('/api/ref-posts/accounts')
  ...
  const fetchPosts = useCallback(async () => {
    ...
    try {
      ...
      params.set('limit', '60')
      const data = await cachedFetch(`/api/ref-posts?${params}`)
      setPosts(data.posts || [])
  ...
  const openPost = async (postId: string) => {
    try {
      const res = await fetch(`/api/ref-posts/${postId}`)
      const data = await res.json()
  ...
  const saveEdit = async () => {
    if (!selectedPost) return
    await fetch(`/api/ref-posts/${selectedPost.id}`, {
      method: 'PATCH',
  ...
  return (
    <>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 24 }}>
        ...
        {/* Post list — each post = horizontal slide strip */}
        {loading ? (
              ...
              >
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <span style={{ fontSize: 11, color: '#666' }}>❤️ {post.like_count?.toLocaleString()}</span>
                    <span style={{ fontSize: 11, color: '#666' }}>💬 {post.comment_count?.toLocaleString()}</span>
      ...
      {/* Detail modal — full-screen slide viewer */}
      {selectedPost && (
        <div
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <span style={{ fontSize: 12, color: '#ccc' }}>❤️ {selectedPost.like_count?.toLocaleString()}</span>
              <span style={{ fontSize: 12, color: '#ccc' }}>💬 {selectedPost.comment_count?.toLocaleString()}</span>
