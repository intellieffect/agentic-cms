...
export default function CollectionsPanel() {
  ...
  const handleRename = async (id: string) => {
    ...
    try {
      await fetch(`/api/references/collections/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/references/collections/${id}`, { method: 'DELETE' })
      load()
    } catch { /* ignore */ }
  }
              ...
              {editingId === col.id ? (
                <div style={{ display: 'flex', gap: 6 }}>
                  <input
                    ...
                    autoFocus
                  />
                  <button onClick={() => handleRename(col.id)} className="btn" style={{ fontSize: 11 }}>✓</button>
                  <button onClick={() => setEditingId(null)} className="btn" style={{ fontSize: 11 }}>✕</button>
                </div>
              ) : (
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    ...
                    <button
                      ...
                      className="btn"
                      style={{ fontSize: 10, padding: '4px 8px', color: '#f88' }}
                    >
                      🗑️
                    </button>
                  </div>
