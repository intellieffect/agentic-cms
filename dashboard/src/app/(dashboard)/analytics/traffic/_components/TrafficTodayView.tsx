...
import type { ChannelBreakdownRow, MetricComparison, PageBreakdownRow, TrafficTodayApiResponse } from "@/lib/analytics/traffic";
    ...
    <div className="rounded-xl border border-[#222] bg-[#141414] p-5">
      ...
      <div className="mt-2 flex items-center justify-between gap-2">
        ...
        <span className="text-xs text-[#666]">
          {metric.previous == null ? "" : `어제 동일 시각 ${kind === "rate" ? fmtPct(metric.previous) : kind === "ratio" ? metric.previous.toFixed(1) : fmtNum(metric.previous)}`}
        </span>
      </div>
    </div>
  );
        ...
        <table className="min-w-full text-sm">
          <thead className="bg-[#111] text-[#777]">
            <tr>
              ...
              <th className="px-4 py-3 text-right font-medium">오늘 세션</th>
              <th className="px-4 py-3 text-right font-medium">어제 세션</th>
              <th className="px-4 py-3 text-right font-medium">오늘 전환</th>
              <th className="px-4 py-3 text-right font-medium">어제 전환</th>
              <th className="px-4 py-3 text-right font-medium">오늘 전환율</th>
              <th className="px-4 py-3 text-right font-medium">어제 전환율</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
...
export function TrafficTodayView({ data }: { data: TrafficTodayApiResponse }) {
  return (
    <div className="flex flex-col gap-6">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
        <StatCard title="오늘 세션" metric={data.summary.sessions} />
        <StatCard title="오늘 순방문자" metric={data.summary.visitors} />
        <StatCard title="오늘 전환 수" metric={data.summary.conversions} />
        <StatCard title="오늘 전환율" metric={data.summary.conversionRate} kind="rate" />
        <StatCard title="오늘 읽힘률" metric={data.summary.readRate} kind="rate" />
      ...
      <section className="rounded-xl border border-[#222] bg-[#141414] p-4">
        <div className="mb-4">
          <div className="text-sm font-medium">오늘 누적 추세</div>
          <div className="text-xs text-[#666]">세션 · 전환 수 누적, 어제 동일 시각 대비</div>
        </div>
        <ResponsiveContainer width="100%" height={340}>
          <LineChart data={data.trends}>
            <CartesianGrid stroke="#222" strokeDasharray="3 3" />
            <XAxis dataKey="label" stroke="#666" tick={{ fill: "#888", fontSize: 11 }} />
            ...
            <Legend />
            <Line yAxisId="count" type="monotone" dataKey="cumulativeSessionsCurrent" name="오늘 누적 세션" stroke="#82ca9d" strokeWidth={2.5} dot={false} />
            <Line yAxisId="count" type="monotone" dataKey="cumulativeSessionsPrevious" name="어제 동일 시각 누적 세션" stroke="#82ca9d" strokeWidth={1.5} dot={false} strokeDasharray="5 5" />
            <Line yAxisId="count" type="monotone" dataKey="cumulativeConversionsCurrent" name="오늘 누적 전환" stroke="#8884d8" strokeWidth={2.5} dot={false} />
            <Line yAxisId="count" type="monotone" dataKey="cumulativeConversionsPrevious" name="어제 동일 시각 누적 전환" stroke="#8884d8" strokeWidth={1.5} dot={false} strokeDasharray="5 5" />
          </LineChart>
        </ResponsiveContainer>
      </section>

      <div className="grid gap-6 xl:grid-cols-2">
        <SimpleTodayTable title="상위 페이지" subtitle="오늘 vs 어제 동일 시각 페이지 비교" rows={data.breakdown.pages} />
        <SimpleTodayTable title="채널 요약" subtitle="기존 채널 분류 요약" rows={data.breakdown.channels} />
      </div>

      <TrafficSourceTable rows={data.breakdown.sources} compareLabel="어제" />
    </div>
  );
}
