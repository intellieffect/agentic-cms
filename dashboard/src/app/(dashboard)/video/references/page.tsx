...
export default function VideoReferencesPage() {
  ...
  const createProjectFromRef = async () => {
    ...
    try {
      ...
      if (projectId) {
        setModalOpen(false);
        router.push(`/video/projects?project=${encodeURIComponent(projectId)}`);
      }
    } catch (e: unknown) {
  ...
  // accounts list is available but sidebar removed — used for filtering only
  void accounts;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
        <h1 className="text-lg font-bold text-foreground">영상 레퍼런스</h1>
        ...
        <div className={styles.content}>
          ...
          <div className="flex items-center gap-4 px-4 py-2 border-b border-border flex-wrap">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                value={platform}
                onChange={(e) => { setPlatform(e.target.value); setCurrentPage(0); }}
              >
          ...
          {totalPages > 1 && (
            <div className={styles.pagination}>
              <button
                className="px-3 py-1 rounded text-xs border border-border bg-muted text-foreground disabled:opacity-40"
          ...
          <>
            <div className={styles.detailSection}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                ...
                <button
                  className="px-1.5 py-0.5 rounded text-[10px] border border-border bg-muted text-foreground"
            ...
            <div className={styles.saveBar}>
              <button
                className="px-3 py-1.5 rounded text-xs border border-border text-foreground"
                style={favorites.has(currentVideo.id) ? { background: '#78350f', borderColor: '#b45309', color: '#fbbf24' } : { background: '#222' }}
