...
export default function CarouselListPage() {
  ...
  const handleDownload = async (id: string, title: string) => {
    setDownloadingId(id)
    try {
      const response = await fetch(`${apiUrl}/api/carousels/${id}/render`, { method: 'POST' })
      if (!response.ok) {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await fetch(`${apiUrl}/api/carousels`)
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`${apiUrl}/api/carousels/${id}`, { method: 'DELETE' })
