...
export default function CarouselDetailPage() {
  ...
  const saveCarousel = useCallback(
    async (next: Partial<Pick<SlideDeckProject, 'title' | 'caption' | 'slides'>>) => {
      ...
      try {
        ...
        const response = await fetch(creating ? `${apiUrl}/api/carousels` : `${apiUrl}/api/carousels/${carousel.id}`, {
        ...
        if (creating && updated?.id) {
          window.history.replaceState({}, '', editorRoute(`/carousel/${updated.id}`))
        }
  ...
  const handleGenerate = useCallback(
    async (source?: GenerateSource) => {
      ...
      try {
        const response = await fetch(`${apiUrl}/api/carousels/generate`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            prompt,
        ...
        if (!carousel || carousel.id === 'new') {
          const createResponse = await fetch(`${apiUrl}/api/carousels`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
    ...
    [generatePrompt, carousel, saveCarousel, apiUrl]
  )
  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      const errMsg = error instanceof Error ? error.message : (error instanceof Event ? 'CORS 또는 이미지 로딩 실패' : String(error))
  ...
  const handleServerRender = useCallback(async (format: 'png' | 'pdf') => {
    ...
    try {
      const endpoint = format === 'pdf'
        ? `${apiUrl}/api/carousels/${carousel.id}/render/pdf`
        : `${apiUrl}/api/carousels/${carousel.id}/render`
      ...
      const errMsg = error instanceof Error ? error.message : (error instanceof Event ? '네트워크 에러' : String(error))
