...
- `DASHBOARD_API_URL`: 뉴스레터 발송 시 dashboard `/api/newsletter/send` 호출에 사용. 기본 `http://localhost:3000`.
...
### 로컬 개발 실행 (dashboard + editor 한 번에)
```bash
cd dashboard && npm run dev:all
```
내부적으로 `concurrently` 가 Next.js dev 서버(3000) 와 Python 영상 편집 서버(8092) 를 동시 기동한다. `EDITOR_API_DIR` / `EDITOR_API_PORT` env 는 `dashboard/.env.local` 에서 관리.

MCP 서버는 Claude Code 가 `.mcp.json` 을 보고 자동으로 spawn 하므로 별도 기동 불필요.
